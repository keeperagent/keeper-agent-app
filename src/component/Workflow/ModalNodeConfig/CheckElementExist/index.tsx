...
import { ICheckElementExistNodeConfig } from "@/electron/type";
...
type Props = {
  onCloseModal: () => any;
  onSaveNodeConfig: (config: ICheckElementExistNodeConfig) => void;
  config: ICheckElementExistNodeConfig;
  isModalOpen: boolean;
};

const CheckElementExist = (props: Props) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      variable: config?.variable || "IS_SWAP_BUTTON_EXIST",
  ...
  const onSubmit = async () => {
    try {
      ...
      onSaveNodeConfig({
        ...
        onError: NODE_ACTION.CONTINUE_RUN,
          ...
          <Fragment>
            ...
            <Form.Item style={{ marginTop: "-0.5rem" }}>
              <Alert
                title={
                  <span>
                    {translate("workflow.checkElementExistHelper1")}{" "}
                    <strong>TRUE</strong>{" "}
                    {translate("workflow.checkElementExistHelper2")}{" "}
                    <strong>FALSE</strong>{" "}
                    {translate("workflow.checkElementExistHelper3")}
                  </span>
                }
                type="info"
                showIcon
...
export default CheckElementExist;
