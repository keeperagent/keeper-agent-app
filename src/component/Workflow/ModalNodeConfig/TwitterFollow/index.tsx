...
type Props = {
  onCloseModal: () => any;
  onSaveNodeConfig: (config: IFollowTwitterNodeConfig) => void;
  config: IFollowTwitterNodeConfig;
  isModalOpen: boolean;
};

const TwitterFollow = (props: Props) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      action: config?.action || FOLLOW_TWITTER_ACTION.FOLLOW,
      sleep: config?.sleep,
      ...
      <Form layout="vertical" form={form} initialValues={{ sleep: 0 }}>
        {activeTab === TAB.DETAIL && (
          <Fragment>
              ...
              label={
                <FormLabelWrapper>
                  <span className="text">{`${translate(
                    "workflow.twitterAccountURL",
                  )}:`}</span>
                  <WorkflowVariable form={form} fieldName="accountUrl" />
                </FormLabelWrapper>
              }
              name="accountUrl"
              rules={[
            ...
            <Form.Item
              label={`${translate("workflow.actionType")}:`}
              name="action"
              rules={[
                {
                  required: true,
            ...
            >
              <Select
                placeholder={translate("workflow.selectActionType")}
                size="large"
                className="custom-select"
                options={[
                  {
                    label: "Follow",
                    value: FOLLOW_TWITTER_ACTION.FOLLOW,
                  },
                  {
                    label: "UnFollow",
                    value: FOLLOW_TWITTER_ACTION.UNFOLLOW,
                  },
                ]}
              />
            </Form.Item>
