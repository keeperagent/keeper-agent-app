...
import { ICloseTabNodeConfig } from "@/electron/type";
import { DEFAULT_TIMEOUT, NODE_ACTION } from "@/electron/simulator/constant";
import { useTranslation } from "@/hook";
import { Wrapper } from "./style";
import { TAB, TAB_NAME_EN } from "@/component/Workflow/ModalNodeConfig/util";
import CommonSetting from "../CommonSetting";
import SkipSetting from "../SkipSetting";

type Props = {
  onCloseModal: () => any;
  onSaveNodeConfig: (config: ICloseTabNodeConfig) => void;
  config: ICloseTabNodeConfig;
  isModalOpen: boolean;
};

const CloseTab = (props: Props) => {
    ...
    <Wrapper>
      ...
      <Form layout="vertical" form={form} initialValues={{ sleep: 0 }}>
        {activeTab === TAB.DETAIL && (
          <Fragment>
            <Form.Item
              label={`${translate("workflow.tabToClose")}:`}
              name="selectedTab"
            >
              <Input
