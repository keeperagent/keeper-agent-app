...
type Props = {
  onCloseModal: () => any;
  onSaveNodeConfig: (config: IClickExtensionNodeConfig) => void;
  config: IClickExtensionNodeConfig;
  isModalOpen: boolean;
};

const ClickExtension = (props: Props) => {
      ...
      <Form layout="vertical" form={form} initialValues={{ sleep: 0 }}>
        {activeTab === TAB.DETAIL && (
          <Fragment>
              ...
              label={
                <FormLabelWrapper>
                  <span className="text">{`${translate(
                    "workflow.elementContent",
                  )}:`}</span>
                  <WorkflowVariable form={form} fieldName="text" />
                </FormLabelWrapper>
              }
              name="text"
              rules={[
            ...
            >
              <Input
                placeholder={translate("workflow.elementContentPlaceholder")}
