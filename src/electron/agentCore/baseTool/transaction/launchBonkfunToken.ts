...
export const launchBonkfunTokenTool = (toolContext?: ToolContext) =>
  new DynamicStructuredTool({
    name: TOOL_KEYS.LAUNCH_BONKFUN_TOKEN,
    description: `Launch a new token on Bonk.fun (Solana only).
...
Optional: imageUrl (URL or local file path), description, twitter, telegram, website, buyAmountSol, slippagePercentage, unitLimit, unitPrice, launchCurrency.`,
    schema: z.object({
      tokenName: z
        .string()
      ...
      unitLimit: z.string().describe("Gas limit (default: '250000')."),
      unitPrice: z
        .string()
        .describe("Gas price in microLamports (default: '100')."),
      launchCurrency: z
        .enum(["SOL", "USD1"])
        .describe("Launch pool currency: SOL or USD1 (default: 'SOL')."),
    }),
    func: async ({
      tokenName,
      symbol,
      ...
      console.log(
        `[launch_bonkfun_token] planState="${toolContext?.planState}" expected="${PlanState.APPROVED}"`,
      );
      if (
      ...
      if (
        ...
        throw new Error(
          "Token launches on Bonk.fun are only supported on Solana blockchain. Please switch to Solana chain in the app first, then try again.",
        );
      }
      ...
      const [txHash, tokenAddress, error] = await bonkfun.createToken(
        ...
        listNodeProvider,
        {
          ...
          launchCurrency:
            launchCurrency === "USD1"
              ? BONKFUN_LAUNCH_CURRENCY.USD1
              : BONKFUN_LAUNCH_CURRENCY.SOL,
        },
      );
      ...
      logEveryWhere({
        message: `[launch_bonkfun_token] tool result: ${toolResult}`,
      });
      return toolResult;
    },
  });
