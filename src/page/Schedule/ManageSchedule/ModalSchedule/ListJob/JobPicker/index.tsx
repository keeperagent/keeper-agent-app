...
import { Select, Form, Tooltip, Tabs, InputNumber } from "antd";
import { connect } from "react-redux";
import { RootState } from "@/redux/store";
import { useTranslation, useGetListCampaign } from "@/hook";
import { ICampaign, IWorkflow, IJob } from "@/electron/type";
...
const JobPicker = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    if (!initialized) {
      form.setFieldsValue({
        secretKey: "", // set value show on UI is empty for security purpose
      });
      setInitialized(true);
    }
  ...
  const onChangeWorkflow = async (value: number) => {
