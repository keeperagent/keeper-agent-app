...
const cacheItemName = "exportCampaignFolderPath";

const ModalExportCampaign = (props: any) => {
  const { translate } = useTranslation();
  const { isModalOpen, setModalOpen, selectedCampaign } = props;

  const { exportCampaignConfig, loading, isSuccess } =
    useExportCampaignConfig();
    ...
    if (isModalOpen) {
      ...
      if (cacheItem) {
        form.setFieldsValue({
          ...
          fileName: "exported_campaign_config",
        });
      } else {
        form.setFieldsValue({
          fileName: "exported_campaign_config",
